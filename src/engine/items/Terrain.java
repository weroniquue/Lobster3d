 ...
 * @author Weronika
 */
//Teren jest zbudowany z bloków, każdy z nich jest jako GameItem z HeightMap
public class Terrain {
    ...
    /**
     * Przeturzymuje granicę dla każdego bloku z terenu
     */
    ...
    public Terrain(int terrainSize, float scale, float minY, float maxY, String heightMapFile, String textureFile, int textInc) throws Exception {
        ...
        PNGDecoder decoder = new PNGDecoder(Terrain.class.getClassLoader().getResourceAsStream("resources/"+heightMapFile));
        ...
        for (int row = 0; row < terrainSize; row++) {
            for (int col = 0; col < terrainSize; col++) {
                ...
                gameItems.add(row * terrainSize + col, terrainBlock);
    ...
    public float getHeight(Vector3f position) {
        ...
        for (int row = 0; row < terrainSize && !found; row++) {
            for (int col = 0; col < terrainSize && !found; col++) {
                terrainBlock = gameItems.get(row * terrainSize + col);
       ...
       System.out.println("Znaleziono wynik: "+ result);
